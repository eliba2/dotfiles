...
**Writing protocol** — after doing real work (a ticket, a code change, a non-trivial investigation with a conclusion), append a bullet by running the quiet helper: `~/.claude/hooks/journal-append.sh "what was done + outcome"`. It appends under `## Worked on` (creating today's file on first use) and prints only `Updated journal`. **Do NOT** use Edit/Write on the journal, read it back, or restate the bullet — that keeps the terminal quiet. A `Stop` hook injects a reminder each turn; treat it as the cue. Don't announce the journal update unless asked.
